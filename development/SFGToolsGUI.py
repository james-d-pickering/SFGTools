...
class TableModel(QtCore.QAbstractTableModel):
    ...
    def list_length(self, lis):
        length = len(lis)
        return length

    def rowCount(self, index):
        return len(max(self.tabledata, key=self.list_length))
    ...
    def add_rows(self, new_rows):
        current_rows = self.rowCount(QtCore.QModelIndex())
        print('current rows = ', current_rows)
        self.beginInsertRows(QtCore.QModelIndex(), current_rows, current_rows+new_rows)
        print('new rows to add =', new_rows)

        self.endInsertRows()
        print('ended insert rows')
...
class MainWindowUIClass(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        if self.initsettings.value("last_upconverter"):
            ...
            self.model.upconversion_line = self.initsettings.value("last_upconverter")

        if self.initsettings.value("last_caliboffset"):
            print(self.initsettings.value("last_caliboffset"))
            print(str(self.initsettings.value("last_caliboffset")))
            self.calibrate_offset_box.setText(str(self.initsettings.value("last_caliboffset")))
            self.model.calibration_offset = np.fromstring( self.initsettings.value("last_caliboffset"), sep=',')
        ...
        self.model.upconversion_line = float(self.upconversion_line_dropdown.currentText())
    ...
    def browse_directorySlot(self):
        ...
        # JDP dont use the native windows dialog as it won't show files.
        self.model.data_directory = dlg.getExistingDirectory(self, 
                                                             caption='Choose directory where data is stored',
                                                             options=dlg.DontUseNativeDialog) + r'/'
    ...
    def browse_write_directorySlot(self):
        ...
        self.model.write_directory = dlg.getExistingDirectory(self,
                                                              caption='Choose directory where output will be saved',
                                                              options=dlg.DontUseNativeDialog) + r'/'
    ...
    def browse_signal_filesSlot(self):
        ...
        filepaths = dlg.getOpenFileNames(self, caption='Choose signal files')[0]
    ...
    def browse_background_filesSlot(self):
        dlg = QtWidgets.QFileDialog()
        dlg.setFileMode(QtWidgets.QFileDialog.ExistingFiles)
        filepaths = dlg.getOpenFileNames(self, caption='Choose background files')[0]
        filenames = []
        for i, file in enumerate(filepaths):
            filename = PurePath(file).name
            filenames.append(filename)
    ...
    def browse_reference_filesSlot(self):
        dlg = QtWidgets.QFileDialog()
        dlg.setFileMode(QtWidgets.QFileDialog.ExistingFiles)
        filepaths = dlg.getOpenFileNames(self, caption='Choose reference files')[0]
        filenames = []
        refnums = []
        for i, file in enumerate(filepaths):
            filename = PurePath(file).name
    ...
    def browse_reference_background_filesSlot(self):
        dlg = QtWidgets.QFileDialog()
        dlg.setFileMode(QtWidgets.QFileDialog.ExistingFiles)
        filepaths = dlg.getOpenFileNames(self, caption='Choose reference background files')[0]
        filenames = []
        for i, file in enumerate(filepaths):
            filename = PurePath(file).name
            filenames.append(filename)
    ...
    def upconversion_line_dropdownSlot(self):
        self.model.upconversion_line = self.upconversion_line_dropdown.currentText()
        if not self.model.upconversion_line == '':
            self.model.upconversion_line = float(self.model.upconversion_line)
        self.initsettings.setValue("last_upconverter", self.model.upconversion_line)
    ...
    def stupid_verboseSlot(self):
        self.model.stupidly_verbose = self.stupid_verbose_check.isChecked()
    ...
    def calibrate_offsetSlot(self):
        self.model.calibration_offset = np.fromstring(self.calibrate_offset_box.text(), sep=',')
        self.initsettings.setValue("last_caliboffset", self.calibrate_offset_box.text())
    ...
    def clear_persistent_settingsSlot(self):
        ...
        if self.initsettings.value("last_upconverter"):
            self.model.upconversion_line = None
            self.upconversion_line_dropdown.setCurrentText(self.model.upconversion_line)
    ...
    def testSlot(self):
        ...
        for i, v in enumerate(itertools.zip_longest(*dat, fillvalue='No File Loaded')):
            print('File ', i+1, ': ', v)
    ...
    @QtCore.pyqtSlot()
    def calibration_sample_dropdownSlot(self):
        self.model.calibration_key = self.calibration_sample_dropdown.currentText()
        self.initsettings.setValue("last_calibration_sample", self.model.calibration_key)
        
    @QtCore.pyqtSlot()
    def calibration_degree_boxSlot(self):
        self.model.calibration_degree = float(self.calibration_degree_box.text())
    
    @QtCore.pyqtSlot()
    def calibrate_buttonSlot(self):
        calibration_data = SFGTools.SFGDataStore()
        calib_file_sig = self.model.data_directory + self.model.signal_names[0]
        self.model.read_files(calib_file_sig, calibration_data, 'sig')
        
        if self.subtract_checkbox.isChecked():
            calib_file_bg = self.model.data_directory + self.model.bg_names[0]
            self.model.read_files(calib_file_bg, calibration_data, 'bg')

        self.model.process_data(calibration_data, self.model.downconvert_check, self.model.subtract_check,
                              self.model.normalise_check, self.model.exposure_check,
                              False, self.model.cosmic_kill_check )
        
        self.model.calibration_sample = self.model.calibration_dict[self.model.calibration_key]
        
        self.model.get_and_set_calibration(calibration_data, self.model.calibration_sample, self.model.calibration_degree)
        self.calibrate_offset_box.setText(np.array2string(self.model.calibration_offset, separator=',')[1:-1])
        return
